...
# Introduction (script 3) 

This script adds columns with categorical enviornmental values to the georeferenced dataframe 'dat_sf' by extracting data from multiple categories of environmental data at each record coordinate. Most environmental data is provided as vector data, but PRISM data are continuous rasters. We bin these continuous abiotic variables into categories so that they can be treated similarly to the vector groups for downstream analyses. 

The input for this script is 'dat_sf' from script 2 and the output is an updated version of 'dat_sf' that includes environmental categorical data (dataframe 'env.v')
...
#Create a raster aoi also, which I'll need for clipping PRISM data; some analyses require this to exist. 
...
load(file="GitHub_script2_georefCleaned.RData")
dim(dat_sf) #geospatial records
...
##Shapefiles 
The first few layers in our biotic and sociopolitical categories are loaded as shapefile (i.e. vector formatted geospatial resources). Later we will read the raster formatted abiotic layers, which requires a different work flow. 
...
```{r}
###Load shapefile of study area (in our case, New Mexico). These are counties.
...
env.v <- dat_sf %>% st_join(left = TRUE, bb.nm["COUNTYFP"])  %>% st_join(left = TRUE, bb.nm["bbnm.area"])
...
#Abiotic data
PRISM data for our study area are in raster format. From these layers, each abiotic variable is continuous across New Mexico. As mentioned earlier, we are going to bin the range of values for each separate abiotic variable (precipitation, temperature and elevation) into discrete categories. This makes it possible to reproduce the analyses for all vector variables (shapefiles above). It does, however, require a bit of data wrangling. 

###PRISM data
IF you haven't worked with PRISM data or the prism API before, we recommend familiarizing yourself with the website and tutorials, etc. 

```{r}
#library(prism) load if you didn't earlier!
...
To get rasters of elevation data we use the [elevatr package](https://github.com/jhollist/elevatr). 
...
#reproject
# Project Raster; to have control,  we create one raster object
prj_dd <- "EPSG:4326"
...
prism_data <- prism_data[complete.cases(prism_data), ] 
...
We also want to know how much area each abiotic zone covers for each of the variables. 

```{r}
####Calulate area in the binned abiotic categories
elevRast <- elevatr::get_elev_raster(locations = aoi, z=9, clip="locations") #just use elevation_data from above if it's loaded; no need to re-run
...
In addition to extract information about the environmental categories in which records occur across the study region, we are also going to characterize the spatial distribution of the point relative to each other. There are many excellent resources available to learn about spatial point processes, and here we will generate a very basic statistic for each record location. We will create a new column that will hold the distance (in meters) to the nearest other record (minimum neighbor distance). 
...
#You could also generate the mean minimum distance from each point to X number of neighbors (the first five in the line below)
#We do not use this in our analysis
env.v <- env.v %>%
