...
In this document, the goal is to produce a cleaned geospatial dataframe of occurrence records. The cleaned dataframe from script 1 ('dat', or whatever you call your dataframe) is the necessary input. You also need a polygon defining the area of interest. We load both a WKT polygon and use the R package AOI; users should choose whatever they want to define their area of interest. Just make sure you get it into sf format and that the projection matches your occurrence data. 

Records with impossible coordinate values are removed, as well as those with incorrect values in stateProvince columns. If you are working on a smaller area, or if you have reason to believe that borderland records should be retained, users should choose the spatial cleaning steps that they need (or do not need) to do. 

Ultimately, the final output of this script ('dat_sf') is the subset of the original cleaned occurrence data ('dat') that are of adequate spatial quality for our purposes.
...
#Coordinate Cleaning
Up until now, we've been cleaning and working with total records from New Mexico based on listed values in the column 'stateProvince'. This is a problematic way to check the spatial locality, but it's what has worked up until now. 
...
#Clean (extra) low precision coordinates
Some of the records (but relatively few) have coordinate uncertainty values that are unacceptable. First, check how many poor quality records are in the data. Then remove anything with uncertainty value above a specific threshold. 
...
We highly recommend users consider all of the spatial flags that GBIF uses to provide information about potential problems in biodiversity data. We also encourage users to make informed decisions about the degree of spatial bias that is acceptable for their purposes. 

Dr. Alexander Zizka's package CoordinateCleaner provides a quantitative way to explore the spatial imprecision in your data;  the [paper](https://doi.org/10.1111/2041-210X.13152) provides more information.

We chose not to remove or rarify records based on proximity to roads, cities, etc. because our objective required keeping all records possible. 
...
save(dat_sf, dat, file="GitHub_script2_georefCleaned.RData")
